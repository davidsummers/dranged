/*
 * Shared main() & friends for test-suite programs.
 *
 */
...
/* Some test programs may want to parse options in the
...
{
  if ( getenv( "DRANGED_TEST_DEBUG" ) )
  {
    volatile bool done = false;
    while ( !done )
    {
      Sleep( 1000 );
      fprintf( stderr, "DRANGED_TEST_DEBUG detected....sleeping.\n" );
    }
  }
  ...
  if ( argc_ >= 2 )  /* notice command-line arguments */
  {
    if ( ! strcmp( argv_[1], "list" ) )
    {
      ran_a_test = 1;
    ...
    else
    {
      for ( i = 1; i < argc_; i++ )
      {
        if ( isdigit( argv_[ i ][ 0 ] ) )
        {
          ran_a_test = 1;
          test_num = atoi (argv_[ i ] );
